...
using GameManager.UIPopup.SmallPopup;
using TMPro;

namespace GameManager
{
    public class SmallPopupManager : MonoBehaviour
    {
        #region Declaration

        private SmallPopup smallPopupPrefab;
        ...
        public void SetupManager(SmallPopup smallPopupPrefab)
        {
            Debug.Log("--- " + this.GetType().Name + ": " + System.Reflection.MethodBase.GetCurrentMethod().Name + " ---");

            this.smallPopupPrefab = smallPopupPrefab;
            popupList = new List<GameObject>();
        }
        ...
        public void OpenPopup(string popupName, TMP_FontAsset fontAsset, TextContentBase.SmallPopup textContent, Action onPrimaryButtonPointerClickCallback, Action onSecondaryButtonPointerClickCallback, Action onAnimationFinishCallback)
        {
        ...
        {
            // Instantiate Popup
            GameObject popup = Instantiate(smallPopupPrefab, popupParent).gameObject;
        ...
        {
            // Find Popup
            SmallPopup popup = popupList.Find(x => x.name == popupName).GetComponent<SmallPopup>();
        ...
        private void SetupPopup(string popupName, TMP_FontAsset fontAsset, TextContentBase.SmallPopup textContent, Action onPrimaryButtonPointerClickCallback, Action onSecondaryButtonPointerClickCallback)
        {
            // Find Popup
            SmallPopup popup = popupList.Find(x => x.name == popupName).GetComponent<SmallPopup>();
